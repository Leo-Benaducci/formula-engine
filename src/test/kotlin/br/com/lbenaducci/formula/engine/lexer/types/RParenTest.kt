...
class RParenTest {
    ...
    inner class Alias {
        ...
        fun `then return alias`() {
            assertEquals("RPAREN", RParen.alias)
        }
    }
    ...
    inner class Matches {
        @Test
        fun `given right parenthesis char, then return true`() {
            assertTrue { RParen.matches(')') }
        }

        @Test
        fun `given other char, then return false`() {
            assertFalse { RParen.matches('a') }
        }
    }
    ...
    inner class Tokenize {
        ...
        fun `given carriage, then return token`() {
            ...
            val token = RParen.tokenize(carriage)

            assertIs<RParen>(token.type)
